...
export function parseTheme(tokens) {
    const lightVars = {};
    const darkVars = {};
    // 1. Parse Light Mode Colors
    if (tokens?.colors?.light) {
        Object.entries(tokens.colors.light).forEach(([category, levels]) => {
    ...
    if (tokens?.colors?.dark) {
        Object.entries(tokens.colors.dark).forEach(([category, levels]) => {
            if (levels && typeof levels === "object") {
                Object.entries(levels).forEach(([level, token]) => {
